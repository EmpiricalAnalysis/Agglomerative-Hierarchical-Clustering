...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        otherCluster = other.points
        
        allDist = []
        
        for i in thisCluster:
            for j in otherCluster:
                #print "points are: "+str(i) + " & "+str(j)
    ...
    def averageLinkageDist(self, other):
        ...
        for i in thisCluster:
            for j in otherCluster:
                allDist += i.distance(j)
                
        mean = allDist/float(len(thisCluster)*len(otherCluster))
    ...
    def members(self):
        for p in self.points:
            yield p
    def isIn(self, name):
        """ Returns True if the element named name is in the cluster
        and False otherwise """
        for p in self.points:
            if p.getName() == name:
                return True
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        for i in c1.members():
            #print "here is member of c1"
            #print i
            allMembers.append(i)
            
        for i in c2.members():
            #print "here is member of c2"
            #print i
            allMembers.append(i)
      
        c12 = Cluster(allMembers, self.pointType)
        newMembers = self.members[:]
        newMembers.remove(c1)
        newMembers.remove(c2)
        newMembers.append(c12)
        self.members = newMembers
        #print "here is member of c12"
        #print i
    ...
    def findClosest(self, linkage):
        ...
        for subset in itertools.combinations(self.members, 2):
            i = subset[0]
            j = subset[1]    
            if count == 0:
                closestDist = linkage(i, j)
                closestClusters = [i, j]
                #print "distance between " + str(i)+" and "+ str(j)+" is "+ str(linkage(i, j))
            elif closestDist > linkage(i, j):
                closestDist = linkage(i, j)
                closestClusters = [i, j]
                #print "distance between " + str(i)+" and "+ str(j)+" is "+ str(linkage(i, j))
            count += 1
        
        #print "closest distance is: " + str(closestDist)

        return closestClusters[0], closestClusters[1]         
    ...
    def mergeOne(self, linkage):
        ...
        #print len(clusterToMerge)
        self.mergeClusters(clusterToMerge[0], clusterToMerge[1])
        
        return clusterToMerge[0], clusterToMerge[1]
...
def readCityData(fName, scale = False):
    ...
    #print len(featureVals), len(featureVals[0])    
    ...
    #print len(featureVectorList), len(featureVectorList[0]) 
    
    return cityNames, featureVectorList

#a, b = readCityData('test.txt', scale = False)
...
def test():
    ...
    #hCluster(points, Cluster.maxLinkageDist, 2, False)
